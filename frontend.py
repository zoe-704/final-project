...
def main():
    ...
    last_pred   = "rock"
    last_conf   = 0.0
    last_probs  = np.array([0.33, 0.33, 0.34])
    inp         = None
    ...
    while True:
        ...
        draw_rounded_rect(canvas, 20, 250, 280, 115, 12, (30, 32, 45), -1)
        put_text(canvas, "DETECTING", (40, 285), 0.6, GRAY)
        put_text(canvas, last_pred.upper(), (40, 335), 1.1, ACCENT, 3)
        conf_w = int(230 * last_conf)
        cv2.rectangle(canvas, (40, 348), (40 + conf_w, 358), ACCENT, -1)
        cv2.rectangle(canvas, (40, 348), (270,          358), GRAY,   1)
        put_text(canvas, f"{last_conf*100:.0f}%", (185, 358), 0.5, GRAY, 1)

        # ── instructions panel ────────────────────────────────────────────────
        draw_rounded_rect(canvas, 20, 385, 280, 105, 12, (30, 32, 45), -1)
        put_text(canvas, "SPACE  play round",  (35, 415), 0.55, GRAY, 1)
        put_text(canvas, "R      reset score", (35, 443), 0.55, GRAY, 1)
        put_text(canvas, "Q      quit",        (35, 471), 0.55, GRAY, 1)
        ...
        if state == "countdown":
            ...
            if remaining > 0:
                put_text(canvas, str(remaining), (W//2 - 40,  H//2 + 60), 5.0, GOLD,  8)
            ...
            draw_rounded_rect(canvas, W//2 - 280, H//2 - 150, 560, 320, 20, (22, 24, 38), -1)
            ...
            put_text(canvas, w_text,                    (W//2 - 130, H//2 - 80), 2.0, w_color, 4)
            put_text(canvas, f"You: {human_move.upper()}", (W//2 - 240, H//2),   0.9, WHITE,   2)
            put_text(canvas, f"AI:  {ai_move.upper()}",    (W//2 + 30,  H//2),   0.9, WHITE,   2)

            # confidence bars for all 3 classes
            put_text(canvas, "Your confidence:", (W//2 - 240, H//2 + 55), 0.55, GRAY, 1)
            for i, name in enumerate(CLASS_NAMES):
                bx  = W//2 - 240 + i * 185
                bw  = int(160 * float(last_probs[i]))
                col = ACCENT if name == human_move else GRAY
                cv2.rectangle(canvas, (bx, H//2 + 75), (bx + bw,  H//2 + 88), col,  -1)
                cv2.rectangle(canvas, (bx, H//2 + 75), (bx + 160, H//2 + 88), GRAY,  1)
                put_text(canvas, name[:3].upper(),             (bx,      H//2 + 108), 0.5, GRAY,  1)
                put_text(canvas, f"{last_probs[i]*100:.0f}%", (bx + 60, H//2 + 108), 0.5, WHITE, 1)

            put_text(canvas, "SPACE for next round", (W//2 - 170, H//2 + 145), 0.65, GRAY, 1)
